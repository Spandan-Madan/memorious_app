...
class AudioUploadManager: ObservableObject {
    ...
    /// Uploads the audio file at the given URL, sending along the recordingId, JWT, and user secret.
                                ...
                                uploadTimer = Timer.scheduledTimer(withTimeInterval: 60, repeats: true) { _ in
                                    if let recId = audioRecorder.recordingId,
                                       let chunkURL = audioRecorder.stopAndAdvanceChunk() {
                                        audioRecorder.currentChunkIndex += 1
...
//        guard let url = URL(string: "https://api.memoriousai.com/userquery") else {
        ...
        Auth.auth().currentUser?.getIDToken { idToken, error in
            guard let idToken = idToken, error == nil else {
                print("Error fetching JWT: \(error?.localizedDescription ?? "Unknown error")")
                DispatchQueue.main.async {
                    messages.append(Message(text: "Error: Authentication failed", isUser: false, isAudio: false))
                }
                return
            }
